...
LINUX64_SHA1=`cat clone-apps-plugin | openssl sha1`
mkdir -p bin/linux64
mv clone-apps-plugin bin/linux64

GOOS=darwin GOARCH=amd64 go build
OSX_SHA1=`cat clone-apps-plugin | openssl sha1`
mkdir -p bin/osx
mv clone-apps-plugin bin/osx

GOOS=windows GOARCH=amd64 go build
WIN64_SHA1=`cat clone-apps-plugin.exe | openssl sha1`
mkdir -p bin/win64
mv clone-apps-plugin.exe bin/win64

GOOS=windows GOARCH=386 go build
WIN32_SHA1=`cat clone-apps-plugin.exe | openssl sha1`
mkdir -p bin/win32
